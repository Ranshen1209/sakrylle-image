...
import { SENTINEL_AGENT_STOPPED, SENTINEL_OPENAI_INTERRUPTED } from './lib/agentSentinels'
...
describe('data import', () => {
  ...
  it('preserves imported task references when restoring config into a non-empty workspace', async () => {
    await clearTasks()
    const localProfile = createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'local-profile', apiKey: 'local-key' })
...
describe('agent draft lifecycle', () => {
  const responsesProfile = createDefaultOpenAIProfile({ baseUrl: 'https://responses.example.com/v1', id: 'openai-responses', apiKey: 'openai-key', apiMode: 'responses' })
  const draftState = {
...
describe('task deletion', () => {
  ...
  it('does not schedule fal recovery after a deleted task rejects late', async () => {
    const request = deferred<Awaited<ReturnType<typeof callImageApi>>>()
    const falProfile = createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'fal-profile', apiKey: 'fal-key' })
    vi.mocked(callImageApi).mockImplementationOnce((opts) => {
      opts.onCustomTaskEnqueued?.({ taskId: 'async-task' })
...
describe('agent built-in image tool failure', () => {
  const responsesProfile = createDefaultOpenAIProfile({ baseUrl: 'https://responses.example.com/v1',
    id: 'responses-profile',
    apiKey: 'test-key',
    apiMode: 'responses',
    model: DEFAULT_RESPONSES_MODEL,
  ...
  it('cleans late response output after an Agent abort', async () => {
    ...
    await vi.waitFor(() => {
      const round = useStore.getState().agentConversations[0].rounds[0]
      expect(round).toMatchObject({ status: 'error', error: SENTINEL_AGENT_STOPPED })
  ...
  it('cleans late response output when Agent execution pauses for recovery', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(recoveryTask?.customRecoverable).toBe(true)
      expect(responseOutput).not.toContain('ig-deleted-recovery')
    })

    const state = useStore.getState()
    const recoveryTask = state.tasks.find((item) => item.agentToolCallId === 'recovery-call')
    expect(recoveryTask).toMatchObject({ status: 'error', customRecoverable: true })
    expect(callAgentResponsesApi).toHaveBeenCalledTimes(1)
...
describe('agent profile routing', () => {
  ...
  it('submits Agent requests with a Responses-mode profile while preserving the Images profile for tools', async () => {
    ...
    expect(call.settings.profiles).toEqual(normalizeSettings({ profiles: [imageProfile] }).profiles)
  })
})
...
describe('reused task API profile', () => {
  ...
  const falProfile = createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'fal-profile', name: 'fal 配置', apiKey: 'fal-key' })
  ...
  it('asks whether to submit with current API profile when the reused API profile is missing', async () => {
    ...
    expect(state.setConfirmDialog).toHaveBeenCalledWith(expect.objectContaining({
      title: '找不到 API 配置',
      message: '找不到复用任务所使用的 API 配置「未知配置」，要使用当前的 API 配置「Default」提交任务吗？',
