    ...
    >
      ...
      >
        ...
        <h3 className="mb-3 pr-8 flex items-start gap-2.5 text-base font-bold text-gray-800 dark:text-gray-100 leading-snug">
          ...
          <span>{i18n.t("upstreamSync.copyImportUrlFor")}{profile.name}{i18n.t("upstreamSync.roundSuffix320")}</span>
        </h3>
        <div className="text-[13px] text-gray-500 dark:text-gray-400 mb-5 leading-relaxed">
          {i18n.t("settings.api.copyUrlMessage")}
        </div>

        <div className="mb-6 rounded-2xl bg-gray-50/80 p-4 dark:bg-white/[0.03] ring-1 ring-black/5 dark:ring-white/5">
          <div className="text-[13px] font-bold text-gray-700 dark:text-gray-300 mb-3.5">{i18n.t("settings.api.newApiVariables")}</div>
          <div className="space-y-3">
            <Checkbox
              checked={options.useNewApiAddress}
              onChange={(checked) => onOptionsChange({ useNewApiAddress: checked })}
              label={<>{i18n.t("settings.api.useVarPrefix")} <code className="mx-0.5 rounded bg-gray-100 px-1.5 py-0.5 text-[0.85em] font-mono text-gray-700 dark:bg-white/[0.08] dark:text-gray-200">{'{address}'}</code> {i18n.t("settings.api.useAddressVarSuffix")}</>}
            />
            <Checkbox
              checked={options.useNewApiKey}
              onChange={(checked) => onOptionsChange({ useNewApiKey: checked })}
              label={<>{i18n.t("settings.api.useVarPrefix")} <code className="mx-0.5 rounded bg-gray-100 px-1.5 py-0.5 text-[0.85em] font-mono text-gray-700 dark:bg-white/[0.08] dark:text-gray-200">{'{key}'}</code></>}
            />
            <Checkbox
              checked={options.useNewApiModel}
              onChange={(checked) => onOptionsChange({ useNewApiModel: checked })}
              label={<>{i18n.t("settings.api.useVarPrefix")} <code className="mx-0.5 rounded bg-gray-100 px-1.5 py-0.5 text-[0.85em] font-mono text-gray-700 dark:bg-white/[0.08] dark:text-gray-200">{'{model}'}</code></>}
            />
          </div>
        </div>
        ...
        <div className="flex gap-2">
          ...
          >
            {i18n.t("settings.api.exclude")}
          </button>
          <button
            onClick={() => onCopy(true)}
            className="flex-1 py-2 rounded-xl bg-[#9181bd] text-white text-sm font-medium hover:bg-[#7e6aa9] transition shadow-sm shadow-[#9181bd]/20"
          >
            {i18n.t("settings.api.include")}
          </button>
        </div>
      </div>
    </div>,
