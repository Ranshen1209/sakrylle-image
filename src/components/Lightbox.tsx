...
export default function Lightbox() {
  ...
  const handleReplaceFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!image) {
        showToast(i18n.t("input.selectValidImage"), 'error')
        return
      }
      ...
      if (targetIdx < 0) {
        ...
        showToast(i18n.t("input.originalReferenceMissing"), 'error')
        return
      }
      if (targetId === image.id) {
        showToast(i18n.t("input.referenceUnchanged"), 'info')
        return
      }
      if (currentImages.some((item, idx) => idx !== targetIdx && item.id === image.id)) {
        showToast(i18n.t("input.referenceAlreadyAdded"), 'info')
        return
      }
      ...
      showToast(i18n.t("input.referenceReplaced"), 'success')
    } catch (err) {
      showToast(i18n.t('upstreamSync.message12', { value0: err instanceof Error ? err.message : String(err) }), 'error')
    }
      ...
      {showInputActions && !isZoomed && (
        <div className="absolute bottom-8 left-1/2 z-10 flex w-max -translate-x-1/2 items-center gap-2 rounded-2xl bg-white/90 dark:bg-black/60 p-2 backdrop-blur-xl border border-gray-200/80 dark:border-white/15 shadow-2xl transition-colors" onClick={(e) => e.stopPropagation()}>
          ...
          >
            ...
            <span>{i18n.t("upstreamSync.replaceImage")}</span>
          </button>
          <div
          ...
          >
            <ButtonTooltip visible={editDisabled && editHint.visible} text={i18n.t("input.onlyOneMask")} />
            <button
              type="button"
              disabled={editDisabled}
              className={`flex items-center justify-center gap-2 whitespace-nowrap rounded-xl px-5 py-2.5 text-sm font-medium shadow-md transition active:scale-95 ${editDisabled ? 'cursor-not-allowed bg-gray-100 text-gray-400 dark:bg-white/10 dark:text-white/40 shadow-none' : 'bg-[#9181bd] text-white hover:bg-[#7e6aa9] hover:shadow-[#9181bd]/25'}`}
              onClick={onEdit}
            >
              <EditIcon className="w-4 h-4" />
              <span>{i18n.t("upstreamSync.editImage")}</span>
            </button>
          </div>
        </div>
      )}
