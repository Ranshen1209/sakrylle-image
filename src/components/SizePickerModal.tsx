...
const SIZE_LIMIT_TEXT = i18n.t("upstreamSync.unsupportedSizesAreAdjustedDimensionsMustBeMultiples")
const CODEX_CLI_SIZE_LIMIT_TEXT = i18n.t("upstreamSync.modelAndCodexCliLimitsDimensionsMustBe")
const CLAMPED_SIZE_TEXT = i18n.t("upstreamSync.resolutionAdjustedToFitModelLimits")
const RATIOS = [
  { label: '1:1', value: '1:1' },
  { label: '3:2', value: '3:2' },
  { label: '2:3', value: '2:3' },
...
export default function SizePickerModal({ currentSize, onSelect, onClose, allowAuto = true, codexCli = false }: Props) {
  ...
  const buttonClass = (active: boolean) => {
    return `rounded-xl border px-3 py-2 text-sm transition ${active
      ? 'border-[#9181bd] bg-[#f1edf8] text-[#7d6cb0] dark:border-[#9181bd]/50 dark:bg-[#9181bd]/10 dark:text-[#c4b8e0]'
      ...
      >
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">{t('size.current', { size: currentSize || 'auto' })}</p>
          </div>
          <button
            onClick={onClose}
            className="rounded-full p-1 text-gray-400 transition hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-white/[0.06] dark:hover:text-gray-200"
            aria-label={t('size.cancel')}
          >
          ...
          <div className="h-[380px] max-h-[55vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-white/10 pr-1 -mr-1 pb-2">
            {mode === 'auto' && (
              <div className="flex h-full animate-fade-in items-center justify-center pt-8 pb-4 text-center">
                <div>
                  <div className="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-full bg-[#f1edf8] text-[#9181bd] dark:bg-[#9181bd]/10">
                  ...
                  <p className="mt-2 text-xs text-gray-400 leading-relaxed dark:text-gray-500">
                    {t('size.autoHintLine1')}
                    <br />
                    {t('size.autoHintLine2')}
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="space-y-5 animate-fade-in">
                      ...
                      return (
                        ...
                        >
                          ...
                          <ViewportTooltip visible={tierHint === item} className="w-52 text-center">
                            {i18n.t("upstreamSync.codexCliSupportsResolutionsUpTo1k")}
                          </ViewportTooltip>
                        </div>
                      )
                    })}
                ...
                {ratio === 'custom' && (
                  <label className="block animate-fade-in">
                    <span className="mb-2 block text-xs font-medium text-gray-400 dark:text-gray-500">{t('size.customRatioLabel')}</span>
                    <input
                      value={customRatio}
                      ...
                      className={`w-full rounded-xl border px-3 py-2 text-sm outline-none transition ${
                        customRatioValid
                          ? 'border-gray-200/70 bg-white/60 text-gray-700 focus:border-[#b9a9da] dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-[#9181bd]/50'
            ...
            {mode === 'resolution' && (
              <div className="space-y-5 animate-fade-in">
                <section>
                  <div className="mb-4 text-xs font-medium text-gray-400 dark:text-gray-500">{t('size.customResolutionTitle')}</div>
                  <div className="flex items-center gap-4">
                    <label className="flex-1">
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2 text-sm text-gray-700 outline-none transition focus:border-[#b9a9da] dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-[#9181bd]/50"
                        placeholder={t('size.examplePixels')}
                      />
                    </label>
                ...
                <div className="rounded-xl border border-gray-200/80 bg-gray-50/80 p-3 text-xs text-gray-600 dark:border-white/[0.05] dark:bg-white/[0.02] dark:text-gray-400">
                  <div className="flex items-start gap-2">
                    <svg className="mt-[2px] h-4 w-4 flex-shrink-0 text-[#9181bd]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="rounded-2xl bg-gray-50 px-4 py-3 dark:bg-white/[0.03]">
            ...
            <div className="mt-1 flex items-center gap-2">
              <span className="font-mono text-lg font-semibold text-gray-800 dark:text-gray-100">
                {previewSize || t('size.invalidSize')}
              </span>
              {isClamped && (
                <div
        ...
        <div className="mt-5 flex gap-2">
          ...
          <button
            ...
            className="flex-1 rounded-xl bg-[#9181bd] px-4 py-2.5 text-sm font-medium text-white transition hover:bg-[#7d6cb0] disabled:cursor-not-allowed disabled:opacity-50"
          >
            {t('size.confirm')}
          </button>
        </div>
