...
// Sakrylle: default fallback is api.sakrylle.com (never openai.com) — CLAUDE.md.
// Production injects VITE_DEFAULT_API_URL so RAW_DEFAULT_API_URL wins; this
// fallback only matters in dev without the env. Upstream's smart derivation
// (importable config URL / Docker proxy) is kept for the custom-provider path.
const SAKRYLLE_DEFAULT_BASE_URL = 'https://api.sakrylle.com/v1'
...
const DEFAULT_API_URL_PATCH = isImportableConfigUrl(RAW_DEFAULT_API_URL)
  ? null
  : parseDefaultApiUrl(RAW_DEFAULT_API_URL || (DOCKER_DEPLOYMENT && DEFAULT_OPENAI_API_PROXY ? '' : SAKRYLLE_DEFAULT_BASE_URL))
...
const BUILT_IN_PROVIDER_IDS = new Set<ApiProvider>(['openai', 'sb2api-async'])
...
const SUB2API_PROVIDER: CustomProviderDefinition = {
  id: 'sb2api-async',
  name: i18n.t("upstreamSync.sub2apiAsync"),
  template: 'http-image',
  submit: {
...
function normalizeProviderOrder(value: unknown, customProviders: CustomProviderDefinition[]): string[] | undefined {
  ...
  const providerIds = ['openai', 'sb2api-async', ...customProviders.map((provider) => provider.id)]
...
export function normalizeCustomProviderDefinition(input: unknown, usedIds = new Set<string>()): CustomProviderDefinition | null {
  ...
  const rawName = typeof record.name === 'string' && record.name.trim() ? record.name.trim() : i18n.t("upstreamSync.customProvider")
...
export function switchApiProfileProvider(profile: ApiProfile, provider: ApiProvider, customProvider?: CustomProviderDefinition): ApiProfile {
  customProvider ??= provider === 'sb2api-async' ? SUB2API_PROVIDER : undefined
  const nextProvider = customProvider ? customProvider.id : 'openai'
  ...
  const targetProvider = customProvider || provider === 'sb2api-async' ? provider : 'openai'
  const savedDraft = providerDrafts[targetProvider]
...
function normalizeProviderDraft(
  ...
  const fallback = createDefaultOpenAIProfile({ transparentBackgroundMethod })
...
export function normalizeApiProfile(
  ...
  const isRemovedFalProvider = rawProvider === 'fal'
  ...
  const providerFallback = customProviderIds.has(provider)
    ...
    : fallback
  const defaults = createDefaultOpenAIProfile({ ...providerFallback, apiMode })
  const rawBaseUrl = !isRemovedFalProvider && typeof record.baseUrl === 'string' ? record.baseUrl : defaults.baseUrl
  ...
  return {
    ...
    provider,
    baseUrl: rawBaseUrl,
    apiKey: !isRemovedFalProvider && typeof record.apiKey === 'string' ? record.apiKey : defaults.apiKey,
    model: !isRemovedFalProvider && typeof record.model === 'string' && record.model.trim() ? record.model : defaults.model,
...
function validateDeploymentProviderIds(input: unknown) {
  ...
  for (const item of input) {
    if (!isRecord(item)) throw new Error(i18n.t("upstreamSync.deploymentProviderConfigurationsMustBeObjects"))
    const name = typeof item.name === 'string' ? item.name.trim() : ''
    const label = name ? `「${name}」` : ''
    const id = typeof item.id === 'string' ? item.id.trim() : ''
    if (!id) throw new Error(i18n.t('upstreamSync.message32', { value0: label }))
    if (BUILT_IN_PROVIDER_IDS.has(id)) throw new Error(i18n.t('upstreamSync.message33', { value0: label, value1: id }))
    if (usedIds.has(id)) throw new Error(i18n.t('upstreamSync.message34', { value0: label, value1: id }))
    if (!normalizeCustomProviderDefinition(item)) throw new Error(i18n.t('upstreamSync.message35', { value0: label }))
    usedIds.add(id)
  }
}
...
function normalizeDeploymentProfileEntries(input: unknown, customProviderIds: Set<string>, nativeTransparentProviderIds = new Set<string>(), deploymentConfig = false) {
  const records = Array.isArray(input)
    ? input.flatMap((item) => {
        ...
        if (!isRecord(item)) throw new Error(i18n.t("upstreamSync.apiConfigurationsMustBeObjects"))
        if (typeof item.provider !== 'string' || (!BUILT_IN_PROVIDER_IDS.has(item.provider) && !customProviderIds.has(item.provider))) {
          const name = typeof item.name === 'string' ? item.name.trim() : ''
          const id = typeof item.id === 'string' ? item.id.trim() : ''
          const label = name || id ? `「${name || id}」` : ''
          throw new Error(i18n.t('upstreamSync.message36', { value0: label }))
        }
        return [item]
      })
    : []
  ...
  if (deploymentConfig && records.length > 1) {
    ...
    if (defaultCount !== 1) throw new Error(i18n.t("upstreamSync.aDeploymentWithMultipleApiConfigurationsMustMark"))
  }
  ...
  for (const record of records) {
    ...
    if (id) {
      if (usedIds.has(id)) {
        ...
        const label = name ? `「${name}」` : ''
        throw new Error(i18n.t('upstreamSync.message37', { value0: label, value1: id }))
      }
      usedIds.add(id)
      continue
    }
...
function validateCustomProviderTaskMappings(providers: CustomProviderDefinition[]) {
  ...
  if (invalid) throw new Error(i18n.t('upstreamSync.message38', { value0: invalid.name }))
}
...
export function importCustomProviderSettingsFromJson(
  ...
  try {
    ...
    throw new Error(i18n.t("upstreamSync.invalidJson"))
  }

  if (!parsed || typeof parsed !== 'object') {
    throw new Error(i18n.t("upstreamSync.theJsonRootMustBeAnObject"))
  }
  ...
  if (Array.isArray(record.customProviders)) {
    ...
    if (customProviders.length === 0) {
      if (!options.deploymentConfig) throw new Error(i18n.t("upstreamSync.noValidProvidersInTheCustomprovidersArray"))
    }
  ...
  throw new Error(i18n.t("upstreamSync.unrecognizedJsonPasteACustomProviderConfiguration"))
}
...
export function getActiveApiProfile(settings: Partial<AppSettings> | unknown): ApiProfile {
  ...
  return {
    ...
    apiKey: typeof record.apiKey === 'string' && record.apiKey.trim() ? record.apiKey : profile.apiKey,
...
export function validateApiProfile(profile: ApiProfile, options: ValidateApiProfileOptions = {}): string | null {
  ...
  if (!profile.baseUrl.trim() && !shouldUseApiProxy(profile.apiProxy)) return i18n.t('errors.profile.missingBaseUrl')
...
function getApiProfileDedupKey(profile: ApiProfile): string {
  return JSON.stringify([
    ...
    profile.apiKey.trim(),
    profile.model.trim(),
    profile.apiMode === 'responses' ? profile.imageGenerationModel?.trim() : undefined,
    profile.apiMode,
...
export function mergeImportedSettings(
  ...
  for (const importedProfile of imported.profiles) {
    ...
    if (hasEquivalentApiProfile(profiles, profile)) continue
...
export const DEFAULT_SETTINGS: AppSettings = normalizeSettings({
  ...
  codexCli: DEFAULT_API_URL_PATCH?.codexCli ?? false,
  apiProxy: DEFAULT_OPENAI_API_PROXY,
  streamImages: DEFAULT_API_URL_PATCH?.streamImages ?? true,
  streamPartialImages: DEFAULT_API_URL_PATCH?.streamPartialImages ?? DEFAULT_STREAM_PARTIAL_IMAGES,
  customProviders: [],
