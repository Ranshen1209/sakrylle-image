import { renderAssistantStatus } from './agentSentinels'
...
async function createUserInputItem(
  ...
  const text = replaceAgentPromptImageReferencesForApi(message.role === 'assistant' ? renderAssistantStatus(message.content) : message.content, round, rounds, tasks)
