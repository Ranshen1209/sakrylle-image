import { SENTINEL_OPENAI_INTERRUPTED } from './agentSentinels'
...
describe('agent conversation state', () => {
  ...
  it('filters malformed and orphaned records while preserving legacy field defaults', () => {
    ...
    expect(normalized[0]).toMatchObject({
      title: '新对话',
      activeRoundId: 'round-a',
      rounds: [{ id: 'round-a', parentRoundId: null, status: 'error', error: SENTINEL_OPENAI_INTERRUPTED }],
    })
...
describe('bilingual mentions after extracting conversation state', () => {
  it('remaps both historical English and Chinese mentions across branch deletion', async () => {
    const { remapAgentRoundMentionsForPathChange } = await import('./agentConversationState')
    const a = round('a', null, 1)
    const b = round('b', 'a', 2)
    expect(remapAgentRoundMentionsForPathChange('@Round2Image1 @第2轮图1', [a, b], [b])).toBe('@第1轮图1 @第1轮图1')
  })
})
