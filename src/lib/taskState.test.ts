...
describe('task lifecycle patches', () => {
  ...
  it('marks legacy and OpenAI running tasks as interrupted', () => {
    ...
    const falRunning = task({ id: 'fal-running', apiProvider: 'sb2api-async', customTaskId: 'async-task', status: 'running', createdAt: 3_000, finishedAt: null, elapsed: null })
    ...
    expect(result.tasks.find((item) => item.id === 'custom-sync-running')).toMatchObject({ status: 'error', error: SENTINEL_OPENAI_INTERRUPTED })
...
describe('task actual params', () => {
  ...
  it('keeps fal and async custom params per image while deriving gallery totals for other providers', () => {
    ...
    expect(deriveGalleryActualParams('sb2api-async', true, { n: 8 }, paramsList, 1)).toEqual(paramsList[0])
