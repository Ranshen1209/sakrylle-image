...
describe('normalizeApiProfile', () => {
  ...
  it('uses provider defaults and preserves explicit transparent background methods', () => {
    ...
    expect(normalizeApiProfile({ provider: 'sb2api-async' }).transparentBackgroundMethod).toBe('api')
    expect(normalizeApiProfile({ provider: 'sb2api-async', transparentBackgroundMethod: 'api' }).transparentBackgroundMethod).toBe('api')
    expect(normalizeApiProfile({ provider: 'sb2api-async', transparentBackgroundMethod: 'invalid' }).transparentBackgroundMethod).toBe('api')
  })
})
...
describe('mergeImportedSettings', () => {
  ...
  it('appends imported profiles as new profiles when current settings are customized', () => {
    ...
    expect(merged.profiles[2]).toMatchObject({ name: 'Imported fal', provider: 'sb2api-async', apiKey: 'fal-key' })
  ...
  it('preserves a new ID even when its connection matches an existing profile', () => {
    ...
    expect(merged.profiles).toHaveLength(3)
    expect(merged.profiles[0]).toMatchObject({ apiKey: 'current-key', model: 'current-model' })
    expect(merged.profiles[1]).toMatchObject({ id: 'duplicate-openai', provider: 'openai', apiKey: 'current-key' })
    expect(merged.profiles[2]).toMatchObject({ id: 'new-openai', provider: 'openai', apiKey: 'imported-key' })
  })
...
describe('mergePresetImportedSettings', () => {
  ...
  it('applies each deployment change once and then keeps later local edits', () => {
    ...
    const current = normalizeSettings({
      ...
      profiles: [
        ...previous.profiles,
        createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'user-profile', name: 'User Profile' }),
      ],
    })
    ...
    expect(merged.profiles.map((profile) => profile.model)).toEqual(['later-local-model', DEFAULT_IMAGES_MODEL])
    expect(merged.profiles[0]).toMatchObject({
      id: 'source-profile',
  ...
  it('preserves the saved profile order while updating locked presets by ID', () => {
    ...
    const user = createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'user-profile' })
    const current = normalizeSettings({
      ...previous,
  ...
  it('uses new provider defaults when a deployment changes the provider type', () => {
    ...
    const nextConfig = importCustomProviderSettingsFromJson(JSON.stringify({
      customProviders: [],
      profiles: [{ id: 'source-profile', provider: 'sb2api-async' }],
    }), [], { deploymentConfig: true })
  ...
  it('keeps the active user profile when updating the deployed config', () => {
    ...
    const userProfile = createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'user-profile', name: 'User Profile' })
    const current = normalizeSettings({
      ...previous,
  ...
  it('does not import preset profiles dismissed by the user', () => {
    const current = normalizeSettings({
      ...DEFAULT_SETTINGS,
      profiles: [createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'user-profile' })],
  ...
  it('respects a filtered preset provider dismissal when locking is enabled', () => {
    ...
    expect(merged.customProviders).toEqual([])
    expect(merged.profiles).toEqual(normalizeSettings({ profiles: [user] }).profiles)
  })
...
describe('default API profile marker', () => {
  ...
  it('preserves profile order when marking the deployment default', () => {
    const userProfile = createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'user-profile' })
...
\`\`\``)
  ...
  it('keeps provider order usable when custom providers are added after manual sorting', () => {
    ...
    expect(settings.providerOrder).toEqual(['openai', 'sb2api-async', 'custom-alpha', 'custom-beta'])
  })
