...
function buildRegularSettingsFromUrlParams(currentSettings: Partial<AppSettings> | unknown, searchParams: URLSearchParams): Partial<AppSettings> {
  ...
  const hasLegacyOpenAIParams = apiKeyParam !== null || codexCliParam !== null || apiMode !== undefined || modelParam !== null || imageGenerationModelParam !== null || profileNameParam !== null || reasoningEffortParam !== null || streamImagesParam !== null || streamPartialImagesParam !== null
  ...
  const requestedNonOpenAIProfile = requestedProfileId
    ...
    : undefined
  if (requestedNonOpenAIProfile && (transparentBackgroundMethod !== undefined || (codexCliParam !== null))) {
    return normalizeSettings({
      ...settings,
      profiles: settings.profiles.map((item) => item.id === requestedProfileId
        ? {
            ...item,
            ...(codexCliParam !== null ? { codexCli: codexCliParam.trim().toLowerCase() === 'true' } : {}),
  ...
  if (hasLegacyOpenAIParams || transparentBackgroundMethod !== undefined) {
    ...
    const profile = createDefaultOpenAIProfile({
      ...
      baseUrl: apiUrlParam !== null ? apiUrlParam.trim() : settings.baseUrl,
      name: i18n.t("url.paramProfileName"),
