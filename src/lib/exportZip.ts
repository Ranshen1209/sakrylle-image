...
export function getExportZipPlan(
  ...
  for (let index = 0; index < plannedTasks.length; index++) {
    addItem(taskBytes[index], i18n.t("upstreamSync.aTaskOrAgentConversationExceeds2Gb"), (part) => part.tasks.push(plannedTasks[index]))
  }
  for (let index = 0; index < plannedConversations.length; index++) {
    addItem(conversationBytes[index], i18n.t("upstreamSync.aTaskOrAgentConversationExceeds2Gb"), (part) => part.agentConversations.push(plannedConversations[index]))
  }
  for (const image of plannedImages) {
    addItem(image.bytes, i18n.t('upstreamSync.message41', { value0: image.id }), (part) => part.imageIds.push(image.id))
  }
  return parts
}
...
export function createExportBlob(bytes: Uint8Array): Blob {
  if (bytes.byteLength >= MAX_EXPORT_ZIP_BYTES) {
    throw new Error(i18n.t("upstreamSync.theBackupExceedsThisBrowserSFileSize"))
  }
...
export async function readExportZipManifest(bytes: Uint8Array, validateFiles = true): Promise<ExportData> {
  ...
  if (!manifestBytes) throw new Error(i18n.t("errors.manifestMissing"))
...
function assertExportZipFiles(manifest: ExportData, hasFile: (path: string) => boolean) {
  ...
  if (missingPath) throw new Error(i18n.t('upstreamSync.message42', { value0: missingPath }))
}
