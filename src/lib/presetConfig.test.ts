...
describe('preset config policy', () => {
  it('exposes every current preset profile in preset-only mode', async () => {
    ...
    policy.setPresetConfig({
      ...
      profiles: [
        ...
        createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'preset-b' }),
      ],
    })
  ...
  it('locks preset parameters and providers except API keys without preventing profile deletion', async () => {
    ...
    const user = createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'user-profile', model: 'user-model' })
  ...
  it('allows removed presets to stay deleted by default', async () => {
    ...
    const presetB = createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'preset-b' })
  ...
  it('locks preset parameters without restoring their deployment order', async () => {
    ...
    const presetB = createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'preset-b', model: 'preset-b-model' })
    const user = createDefaultOpenAIProfile({ id: 'user-profile' })
    policy.setPresetConfig({ customProviders: [], profiles: [presetA, presetB] })
  ...
  it('uses the default preset URL for supported synchronous and asynchronous providers', async () => {
    const { createDefaultOpenAIProfile } = await import('./apiProfiles')
    const policy = await import('./presetConfig')
    policy.setPresetConfig({
      customProviders: [],
    ...
    policy.setPresetConfig({
      customProviders: [],
      profiles: [createDefaultOpenAIProfile({ provider: 'sb2api-async',  id: 'fal-preset', baseUrl: 'https://fal-proxy.example.com' })],
    })
    expect(policy.getDefaultPresetBaseUrl()).toBe('https://fal-proxy.example.com')
  })
})
