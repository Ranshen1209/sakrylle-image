import { isAgentStoppedSentinel } from './agentSentinels'
...
function markToolStatusStopped(status: AgentWebSearchStatus): AgentWebSearchStatus {
  ...
  return { text: i18n.language.startsWith('zh') ? status.text.replace(/^正在/, '已停止') : `${i18n.t('agent.stopped')} ${status.text}`, completed: true }
}
